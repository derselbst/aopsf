...
static const uint8_t mips_reg_layout[] =
{
	MIPS_PC, -1,
...
static const uint8_t mips_win_layout[] = {
...
static void setcp2dr( MIPS_CPU_CONTEXT *mipscpu, int n_reg, uint32_t n_value );
...
INLINE void mips_delayed_branch( MIPS_CPU_CONTEXT *mipscpu, uint32_t n_adr )
{
...
INLINE void mips_load( MIPS_CPU_CONTEXT *mipscpu, uint32_t n_r, uint32_t n_v )
{
...
INLINE void mips_delayed_load( MIPS_CPU_CONTEXT *mipscpu, uint32_t n_r, uint32_t n_v )
{
	if( mipscpu->delayr == REGPC )
	{
		mips_set_pc( mipscpu, mipscpu->delayv );
...
static void mips_exception( MIPS_CPU_CONTEXT *mipscpu, int exception )
{
	...
	if( mipscpu->delayr == REGPC )
	{
		...
		mips_set_cp0r( mipscpu, CP0_CAUSE, ( mipscpu->cp0r[ CP0_CAUSE ] & ~CAUSE_EXC ) | CAUSE_BD | ( exception << 2 ) );
	}
	else
	{
		mips_commit_delayed_load(mipscpu);
...
void mips_init( MIPS_CPU_CONTEXT *mipscpu )
{
	...
	state_save_register_uint32_t( "psxcpu", cpu, "op", &mipscpu->op, 1 );
	state_save_register_uint32_t( "psxcpu", cpu, "pc", &mipscpu->pc, 1 );
	state_save_register_uint32_t( "psxcpu", cpu, "delayv", &mipscpu->delayv, 1 );
	state_save_register_uint32_t( "psxcpu", cpu, "delayr", &mipscpu->delayr, 1 );
	state_save_register_uint32_t( "psxcpu", cpu, "hi", &mipscpu->hi, 1 );
	state_save_register_uint32_t( "psxcpu", cpu, "lo", &mipscpu->lo, 1 );
	state_save_register_uint32_t( "psxcpu", cpu, "r", &mipscpu->r[ 0 ], 32 );
	state_save_register_uint32_t( "psxcpu", cpu, "cp0r", &mipscpu->cp0r[ 0 ], 32 );
	state_save_register_uint32_t( "psxcpu", cpu, "cp2cr", &mipscpu->cp2cr[ 0 ].d, 32 );
	state_save_register_uint32_t( "psxcpu", cpu, "cp2dr", &mipscpu->cp2dr[ 0 ].d, 32 );
#endif
}

void mips_reset( MIPS_CPU_CONTEXT *mipscpu, void *param )
{
	mips_set_cp0r( mipscpu, CP0_SR, ( mipscpu->cp0r[ CP0_SR ] & ~( SR_TS | SR_SWC | SR_KUC | SR_IEC ) ) | SR_BEV );
...
//				printf("HLECALL, PC = %08x\n", mipscpu->pc);
				...
				break;
			case FUNCT_SLL:
				mips_load( mipscpu, INS_RD( mipscpu->op ), mipscpu->r[ INS_RT( mipscpu->op ) ] << INS_SHAMT( mipscpu->op ) );
				break;
			case FUNCT_SRL:
				mips_load( mipscpu, INS_RD( mipscpu->op ), mipscpu->r[ INS_RT( mipscpu->op ) ] >> INS_SHAMT( mipscpu->op ) );
				break;
			case FUNCT_SRA:
				mips_load( mipscpu, INS_RD( mipscpu->op ), (int32_t)mipscpu->r[ INS_RT( mipscpu->op ) ] >> INS_SHAMT( mipscpu->op ) );
				break;
			case FUNCT_SLLV:
				mips_load( mipscpu, INS_RD( mipscpu->op ), mipscpu->r[ INS_RT( mipscpu->op ) ] << ( mipscpu->r[ INS_RS( mipscpu->op ) ] & 31 ) );
				break;
			case FUNCT_SRLV:
				mips_load( mipscpu, INS_RD( mipscpu->op ), mipscpu->r[ INS_RT( mipscpu->op ) ] >> ( mipscpu->r[ INS_RS( mipscpu->op ) ] & 31 ) );
				break;
			case FUNCT_SRAV:
				mips_load( mipscpu, INS_RD( mipscpu->op ), (int32_t)mipscpu->r[ INS_RT( mipscpu->op ) ] >> ( mipscpu->r[ INS_RS( mipscpu->op ) ] & 31 ) );
				break;
			case FUNCT_JR:
...
//				mips_exception( EXC_BP );
			...
			case FUNCT_ADD:
				...
				break;
			case FUNCT_ADDU:
				mips_load( mipscpu, INS_RD( mipscpu->op ), mipscpu->r[ INS_RS( mipscpu->op ) ] + mipscpu->r[ INS_RT( mipscpu->op ) ] );
				break;
			case FUNCT_SUB:
				n_res = mipscpu->r[ INS_RS( mipscpu->op ) ] - mipscpu->r[ INS_RT( mipscpu->op ) ];
				if( (int32_t)( ( mipscpu->r[ INS_RS( mipscpu->op ) ] ^ mipscpu->r[ INS_RT( mipscpu->op ) ] ) & ( mipscpu->r[ INS_RS( mipscpu->op ) ] ^ n_res ) ) < 0 )
			...
			case FUNCT_NOR:
				...
				break;
			case FUNCT_SLT:
				mips_load( mipscpu, INS_RD( mipscpu->op ), (int32_t)mipscpu->r[ INS_RS( mipscpu->op ) ] < (int32_t)mipscpu->r[ INS_RT( mipscpu->op ) ] );
				break;
			case FUNCT_SLTU:
				mips_load( mipscpu, INS_RD( mipscpu->op ), mipscpu->r[ INS_RS( mipscpu->op ) ] < mipscpu->r[ INS_RT( mipscpu->op ) ] );
				break;
		...
		case OP_REGIMM:
			...
			case RT_BLTZ:
				...
				{
					mips_delayed_branch( mipscpu, mipscpu->pc + 4 + ( MIPS_WORD_EXTEND( INS_IMMEDIATE( mipscpu->op ) ) << 2 ) );
				}
				else
				{
					mips_advance_pc(mipscpu);
			...
			break;
		case OP_J:
			mips_delayed_branch( mipscpu, ( ( mipscpu->pc + 4 ) & 0xf0000000 ) + ( INS_TARGET( mipscpu->op ) << 2 ) );
			break;
		case OP_JAL:
			n_res = mipscpu->pc + 8;
			mips_delayed_branch( mipscpu, ( ( mipscpu->pc + 4 ) & 0xf0000000 ) + ( INS_TARGET( mipscpu->op ) << 2 ) );
		...
		case OP_ADDIU:
			...
			{
				mips_load( mipscpu, INS_RT( mipscpu->op ), mipscpu->r[ INS_RS( mipscpu->op ) ] + MIPS_WORD_EXTEND( INS_IMMEDIATE( mipscpu->op ) ) );
			}
			break;
		case OP_SLTI:
			mips_load( mipscpu, INS_RT( mipscpu->op ), (int32_t)mipscpu->r[ INS_RS( mipscpu->op ) ] < MIPS_WORD_EXTEND( INS_IMMEDIATE( mipscpu->op ) ) );
			break;
		case OP_SLTIU:
			mips_load( mipscpu, INS_RT( mipscpu->op ), mipscpu->r[ INS_RS( mipscpu->op ) ] < (uint32_t)MIPS_WORD_EXTEND( INS_IMMEDIATE( mipscpu->op ) ) );
			break;
		case OP_ANDI:
			mips_load( mipscpu, INS_RT( mipscpu->op ), mipscpu->r[ INS_RS( mipscpu->op ) ] & INS_IMMEDIATE( mipscpu->op ) );
			break;
		case OP_ORI:
			mips_load( mipscpu, INS_RT( mipscpu->op ), mipscpu->r[ INS_RS( mipscpu->op ) ] | INS_IMMEDIATE( mipscpu->op ) );
			break;
		...
		case OP_COP0:
			if( ( mipscpu->cp0r[ CP0_SR ] & SR_KUC ) != 0 && ( mipscpu->cp0r[ CP0_SR ] & SR_CU0 ) == 0 )
			{
				...
				mips_set_cp0r( mipscpu, CP0_CAUSE, ( mipscpu->cp0r[ CP0_CAUSE ] & ~CAUSE_CE ) | CAUSE_CE0 );
			}
			else
			{
				...
				{
				case RS_MFC:
					mips_delayed_load( mipscpu, INS_RT( mipscpu->op ), mipscpu->cp0r[ INS_RD( mipscpu->op ) ] );
					break;
				case RS_CFC:
					/* todo: */
				...
				default:
					...
					case 1:
						...
						case CF_RFE:
							mips_advance_pc(mipscpu);
							mips_set_cp0r( mipscpu, CP0_SR, ( mipscpu->cp0r[ CP0_SR ] & ~0xf ) | ( ( mipscpu->cp0r[ CP0_SR ] >> 2 ) & 0xf ) );
							break;
						default:
							/* todo: */
							logerror( mipscpu, "%08x: COP0 unknown command %08x\n", mipscpu->pc, mipscpu->op );
		...
		case OP_COP1:
			...
			{
				mips_exception( mipscpu, EXC_CPU );
				mips_set_cp0r( mipscpu, CP0_CAUSE, ( mipscpu->cp0r[ CP0_CAUSE ] & ~CAUSE_CE ) | CAUSE_CE1 );
			}
			else
			{
				switch( INS_RS( mipscpu->op ) )
		...
		case OP_COP2:
			...
			{
				mips_exception( mipscpu, EXC_CPU );
				mips_set_cp0r( mipscpu, CP0_CAUSE, ( mipscpu->cp0r[ CP0_CAUSE ] & ~CAUSE_CE ) | CAUSE_CE2 );
			}
			else
			{
				switch( INS_RS( mipscpu->op ) )
				{
				case RS_MFC:
					mips_delayed_load( mipscpu, INS_RT( mipscpu->op ), getcp2dr( mipscpu, INS_RD( mipscpu->op ) ) );
					break;
				case RS_CFC:
					mips_delayed_load( mipscpu, INS_RT( mipscpu->op ), getcp2cr( mipscpu, INS_RD( mipscpu->op ) ) );
					break;
				case RS_MTC:
		...
		case OP_LB:
			...
			else if( ( mipscpu->cp0r[ CP0_SR ] & ( SR_RE | SR_KUC ) ) == ( SR_RE | SR_KUC ) )
			{
				...
				{
					mips_delayed_load( mipscpu, INS_RT( mipscpu->op ), MIPS_BYTE_EXTEND( program_read_byte_32le( mipscpu->psx, n_adr ^ 3 ) ) );
				}
			}
			else
			{
				...
				{
					mips_delayed_load( mipscpu, INS_RT( mipscpu->op ), MIPS_BYTE_EXTEND( program_read_byte_32le( mipscpu->psx, n_adr ) ) );
				}
			}
			break;
		case OP_LH:
			...
			else if( ( mipscpu->cp0r[ CP0_SR ] & ( SR_RE | SR_KUC ) ) == ( SR_RE | SR_KUC ) )
			{
				...
				{
					mips_delayed_load( mipscpu, INS_RT( mipscpu->op ), MIPS_WORD_EXTEND( program_read_word_32le( mipscpu->psx, n_adr ^ 2 ) ) );
				}
			}
			else
			{
				...
				{
					mips_delayed_load( mipscpu, INS_RT( mipscpu->op ), MIPS_WORD_EXTEND( program_read_word_32le( mipscpu->psx, n_adr ) ) );
				}
			}
			break;
		case OP_LWL:
			...
			else if( ( mipscpu->cp0r[ CP0_SR ] & ( SR_RE | SR_KUC ) ) == ( SR_RE | SR_KUC ) )
			{
				...
				else
				{
					...
					{
					case 0:
						n_res = ( mipscpu->r[ INS_RT( mipscpu->op ) ] & 0x00ffffff ) | ( (uint32_t)program_read_byte_32le( mipscpu->psx, n_adr + 3 ) << 24 );
						break;
					case 1:
						n_res = ( mipscpu->r[ INS_RT( mipscpu->op ) ] & 0x0000ffff ) | ( (uint32_t)program_read_word_32le( mipscpu->psx, n_adr + 1 ) << 16 );
						break;
					case 2:
						n_res = ( mipscpu->r[ INS_RT( mipscpu->op ) ] & 0x000000ff ) | ( (uint32_t)program_read_byte_32le( mipscpu->psx, n_adr - 1 ) << 8 ) | ( (uint32_t)program_read_word_32le( mipscpu->psx, n_adr ) << 16 );
						break;
					default:
			...
			else
			{
				...
				else
				{
					...
					{
					case 0:
						n_res = ( mipscpu->r[ INS_RT( mipscpu->op ) ] & 0x00ffffff ) | ( (uint32_t)program_read_byte_32le( mipscpu->psx, n_adr ) << 24 );
						break;
					case 1:
						n_res = ( mipscpu->r[ INS_RT( mipscpu->op ) ] & 0x0000ffff ) | ( (uint32_t)program_read_word_32le( mipscpu->psx, n_adr - 1 ) << 16 );
						break;
					case 2:
						n_res = ( mipscpu->r[ INS_RT( mipscpu->op ) ] & 0x000000ff ) | ( (uint32_t)program_read_word_32le( mipscpu->psx, n_adr - 2 ) << 8 ) | ( (uint32_t)program_read_byte_32le( mipscpu->psx, n_adr ) << 24 );
						break;
					default:
						n_res = program_read_dword_32le( mipscpu->psx, n_adr - 3 );
						break;
...
#endif
				{
					mips_delayed_load( mipscpu, INS_RT( mipscpu->op ), program_read_dword_32le( mipscpu->psx, n_adr ) );
				}
			}
			break;
		case OP_LBU:
			...
			else if( ( mipscpu->cp0r[ CP0_SR ] & ( SR_RE | SR_KUC ) ) == ( SR_RE | SR_KUC ) )
			{
				...
				{
					mips_delayed_load( mipscpu, INS_RT( mipscpu->op ), program_read_byte_32le( mipscpu->psx, n_adr ^ 3 ) );
				}
			}
			else
			{
				...
				{
					mips_delayed_load( mipscpu, INS_RT( mipscpu->op ), program_read_byte_32le( mipscpu->psx, n_adr ) );
				}
			}
			break;
		case OP_LHU:
			...
			else if( ( mipscpu->cp0r[ CP0_SR ] & ( SR_RE | SR_KUC ) ) == ( SR_RE | SR_KUC ) )
			{
				...
				{
					mips_delayed_load( mipscpu, INS_RT( mipscpu->op ), program_read_word_32le( mipscpu->psx, n_adr ^ 2 ) );
				}
			}
			else
			{
				...
				{
					mips_delayed_load( mipscpu, INS_RT( mipscpu->op ), program_read_word_32le( mipscpu->psx, n_adr ) );
				}
			}
			break;
		case OP_LWR:
			...
			else if( ( mipscpu->cp0r[ CP0_SR ] & ( SR_RE | SR_KUC ) ) == ( SR_RE | SR_KUC ) )
			{
				...
				else
				{
					...
					{
					case 3:
						n_res = ( mipscpu->r[ INS_RT( mipscpu->op ) ] & 0xffffff00 ) | program_read_byte_32le( mipscpu->psx, n_adr - 3 );
						break;
					case 2:
						n_res = ( mipscpu->r[ INS_RT( mipscpu->op ) ] & 0xffff0000 ) | program_read_word_32le( mipscpu->psx, n_adr - 2 );
						break;
					case 1:
						n_res = ( mipscpu->r[ INS_RT( mipscpu->op ) ] & 0xff000000 ) | program_read_word_32le( mipscpu->psx, n_adr - 1 ) | ( (uint32_t)program_read_byte_32le( mipscpu->psx, n_adr + 1 ) << 16 );
						break;
					default:
			...
			else
			{
				...
				else
				{
					...
					{
					case 3:
						n_res = ( mipscpu->r[ INS_RT( mipscpu->op ) ] & 0xffffff00 ) | program_read_byte_32le( mipscpu->psx, n_adr );
						break;
					case 2:
						n_res = ( mipscpu->r[ INS_RT( mipscpu->op ) ] & 0xffff0000 ) | program_read_word_32le( mipscpu->psx, n_adr );
						break;
					case 1:
						n_res = ( mipscpu->r[ INS_RT( mipscpu->op ) ] & 0xff000000 ) | program_read_byte_32le( mipscpu->psx, n_adr ) | ( (uint32_t)program_read_word_32le( mipscpu->psx, n_adr + 1 ) << 8 );
						break;
					default:
						n_res = program_read_dword_32le( mipscpu->psx, n_adr );
						break;
		...
		case OP_SB:
			...
			else if( ( mipscpu->cp0r[ CP0_SR ] & ( SR_RE | SR_KUC ) ) == ( SR_RE | SR_KUC ) )
			{
				...
				{
					program_write_byte_32le( mipscpu->psx, n_adr ^ 3, mipscpu->r[ INS_RT( mipscpu->op ) ] );
					mips_advance_pc(mipscpu);
				}
			}
			else
			{
				...
				{
					program_write_byte_32le( mipscpu->psx, n_adr, mipscpu->r[ INS_RT( mipscpu->op ) ] );
					mips_advance_pc(mipscpu);
				}
			}
			break;
		case OP_SH:
			...
			else if( ( mipscpu->cp0r[ CP0_SR ] & ( SR_RE | SR_KUC ) ) == ( SR_RE | SR_KUC ) )
			{
				...
				{
					program_write_word_32le( mipscpu->psx, n_adr ^ 2, mipscpu->r[ INS_RT( mipscpu->op ) ] );
					mips_advance_pc(mipscpu);
				}
			}
			else
			{
				...
				{
					program_write_word_32le( mipscpu->psx, n_adr, mipscpu->r[ INS_RT( mipscpu->op ) ] );
					mips_advance_pc(mipscpu);
				}
			}
			break;
		case OP_SWL:
			...
			else if( ( mipscpu->cp0r[ CP0_SR ] & ( SR_RE | SR_KUC ) ) == ( SR_RE | SR_KUC ) )
			{
				...
				else
				{
					...
					{
					case 0:
						program_write_byte_32le( mipscpu->psx, n_adr + 3, mipscpu->r[ INS_RT( mipscpu->op ) ] >> 24 );
						break;
					case 1:
						program_write_word_32le( mipscpu->psx, n_adr + 1, mipscpu->r[ INS_RT( mipscpu->op ) ] >> 16 );
						break;
					case 2:
						program_write_byte_32le( mipscpu->psx, n_adr - 1, mipscpu->r[ INS_RT( mipscpu->op ) ] >> 8 );
						program_write_word_32le( mipscpu->psx, n_adr, mipscpu->r[ INS_RT( mipscpu->op ) ] >> 16 );
						break;
					case 3:
						program_write_dword_32le( mipscpu->psx, n_adr - 3, mipscpu->r[ INS_RT( mipscpu->op ) ] );
						break;
					}
					mips_advance_pc(mipscpu);
				}
			...
			else
			{
				...
				else
				{
					...
					{
					case 0:
						program_write_byte_32le( mipscpu->psx, n_adr, mipscpu->r[ INS_RT( mipscpu->op ) ] >> 24 );
						break;
					case 1:
						program_write_word_32le( mipscpu->psx, n_adr - 1, mipscpu->r[ INS_RT( mipscpu->op ) ] >> 16 );
						break;
					case 2:
						program_write_word_32le( mipscpu->psx, n_adr - 2, mipscpu->r[ INS_RT( mipscpu->op ) ] >> 8 );
						program_write_byte_32le( mipscpu->psx, n_adr, mipscpu->r[ INS_RT( mipscpu->op ) ] >> 24 );
						break;
					case 3:
						program_write_dword_32le( mipscpu->psx, n_adr - 3, mipscpu->r[ INS_RT( mipscpu->op ) ] );
...
*/
			...
			else
			{
				...
				{
					program_write_dword_32le( mipscpu->psx, n_adr, mipscpu->r[ INS_RT( mipscpu->op ) ] );
					mips_advance_pc(mipscpu);
				}
			}
			break;
		case OP_SWR:
			...
			else if( ( mipscpu->cp0r[ CP0_SR ] & ( SR_RE | SR_KUC ) ) == ( SR_RE | SR_KUC ) )
			{
				...
				else
				{
					...
					case 0:
						...
						break;
					case 1:
						program_write_word_32le( mipscpu->psx, n_adr - 1, mipscpu->r[ INS_RT( mipscpu->op ) ] );
						program_write_byte_32le( mipscpu->psx, n_adr + 1, mipscpu->r[ INS_RT( mipscpu->op ) ] >> 16 );
						break;
					case 2:
						program_write_word_32le( mipscpu->psx, n_adr - 2, mipscpu->r[ INS_RT( mipscpu->op ) ] );
						break;
			...
			else
			{
				...
				else
				{
					...
					case 1:
						program_write_byte_32le( mipscpu->psx, n_adr, mipscpu->r[ INS_RT( mipscpu->op ) ] );
						program_write_word_32le( mipscpu->psx, n_adr + 1, mipscpu->r[ INS_RT( mipscpu->op ) ] >> 8 );
						break;
					case 2:
						program_write_word_32le( mipscpu->psx, n_adr, mipscpu->r[ INS_RT( mipscpu->op ) ] );
						break;
		...
		case OP_LWC2:
			...
			else
			{
				...
				{
					/* todo: delay? */
					setcp2dr( mipscpu, INS_RT( mipscpu->op ), program_read_dword_32le( mipscpu->psx, n_adr ) );
					mips_advance_pc(mipscpu);
				}
			}
			break;
		...
		case OP_SWC2:
			...
			else
			{
				...
				{
					program_write_dword_32le( mipscpu->psx, n_adr, getcp2dr( mipscpu, INS_RT( mipscpu->op ) ) );
					mips_advance_pc(mipscpu);
				}
			}
			break;
...
INLINE int32_t LIM( MIPS_CPU_CONTEXT *mipscpu, int32_t n_value, int32_t n_max, int32_t n_min, uint32_t n_flag )
{
	if( n_value > n_max )
	{
		FLAG |= n_flag;
...
INLINE int64_t BOUNDS( MIPS_CPU_CONTEXT *mipscpu, int64_t n_value, int64_t n_max, int n_maxflag, int64_t n_min, int n_minflag )
{
	if( n_value > n_max )
	{
		FLAG |= n_maxflag;
...
#define A1( a ) BOUNDS( mipscpu, ( a ), 0x7fffffff, 30, -(int64_t)0x80000000, ( 1 << 27 ) )
#define A2( a ) BOUNDS( mipscpu, ( a ), 0x7fffffff, 29, -(int64_t)0x80000000, ( 1 << 26 ) )
#define A3( a ) BOUNDS( mipscpu, ( a ), 0x7fffffff, 28, -(int64_t)0x80000000, ( 1 << 25 ) )
...
INLINE uint32_t Lm_E( MIPS_CPU_CONTEXT *mipscpu, uint32_t n_z )
{
	if( n_z <= H / 2 )
	{
		n_z = H / 2;
...
#define F( a ) BOUNDS( mipscpu, ( a ), 0x7fffffff, ( 1 << 31 ) | ( 1 << 16 ), -(int64_t)0x80000000, ( 1 << 31 ) | ( 1 << 15 ) )
...
static void docop2( MIPS_CPU_CONTEXT *mipscpu, int gteop )
{
	...
	const uint16_t n_zm = 0;
	const uint32_t n_zc = 0;
	const uint16_t *p_n_vx[] = { &VX0, &VX1, &VX2 };
	const uint16_t *p_n_vy[] = { &VY0, &VY1, &VY2 };
	const uint16_t *p_n_vz[] = { &VZ0, &VZ1, &VZ2 };
	const uint16_t *p_n_rm[] = { &R11, &R12, &R13, &R21, &R22, &R23, &R31, &R32, &R33 };
	const uint16_t *p_n_lm[] = { &L11, &L12, &L13, &L21, &L22, &L23, &L31, &L32, &L33 };
	const uint16_t *p_n_cm[] = { &LR1, &LR2, &LR3, &LG1, &LG2, &LG3, &LB1, &LB2, &LB3 };
	const uint16_t *p_n_zm[] = { &n_zm, &n_zm, &n_zm, &n_zm, &n_zm, &n_zm, &n_zm, &n_zm, &n_zm };
	const uint16_t **p_p_n_mx[] = { p_n_rm, p_n_lm, p_n_cm, p_n_zm };
	const uint32_t *p_n_tr[] = { &TRX, &TRY, &TRZ };
	const uint32_t *p_n_bk[] = { &RBK, &GBK, &BBK };
	const uint32_t *p_n_fc[] = { &RFC, &GFC, &BFC };
	const uint32_t *p_n_zc[] = { &n_zc, &n_zc, &n_zc };
	const uint32_t **p_p_n_cv[] = { p_n_tr, p_n_bk, p_n_fc, p_n_zc };
	...
	case 0x01:
		if( gteop == 0x0180001 )
		{
			...
			SXY0 = SXY1;
			SXY1 = SXY2;
			SX2 = Lm_G1( F( (int64_t)(int32_t)OFX + ( (int64_t)(int16_t)IR1 * ( ( (uint32_t)H << 16 ) / Lm_E( mipscpu, SZ3 ) ) ) ) >> 16 );
			SY2 = Lm_G2( F( (int64_t)(int32_t)OFY + ( (int64_t)(int16_t)IR2 * ( ( (uint32_t)H << 16 ) / Lm_E( mipscpu, SZ3 ) ) ) ) >> 16 );
			MAC0 = F( (int64_t)(int32_t)DQB + ( (int64_t)(int16_t)DQA * ( ( (uint32_t)H << 16 ) / Lm_E( mipscpu, SZ3 ) ) ) );
...
void mips_set_info(MIPS_CPU_CONTEXT *mipscpu, uint32_t state, union cpuinfo *info)
{
	switch (state)
	{
		/* --- the following bits of info are set as 64-bit signed integers --- */
		case CPUINFO_INT_INPUT_STATE + MIPS_IRQ0:		set_irq_line(mipscpu, MIPS_IRQ0, info->i);		break;
		case CPUINFO_INT_INPUT_STATE + MIPS_IRQ1:		set_irq_line(mipscpu, MIPS_IRQ1, info->i);		break;
		case CPUINFO_INT_INPUT_STATE + MIPS_IRQ2:		set_irq_line(mipscpu, MIPS_IRQ2, info->i);		break;
		case CPUINFO_INT_INPUT_STATE + MIPS_IRQ3:		set_irq_line(mipscpu, MIPS_IRQ3, info->i);		break;
		case CPUINFO_INT_INPUT_STATE + MIPS_IRQ4:		set_irq_line(mipscpu, MIPS_IRQ4, info->i);		break;
		case CPUINFO_INT_INPUT_STATE + MIPS_IRQ5:		set_irq_line(mipscpu, MIPS_IRQ5, info->i);		break;

		case CPUINFO_INT_PC:							mips_set_pc( mipscpu, info->i );					break;
		case CPUINFO_INT_REGISTER + MIPS_PC:			mips_set_pc( mipscpu, info->i );					break;
		...
		case CPUINFO_INT_REGISTER + MIPS_CP0R0:			mips_set_cp0r( mipscpu, 0, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R1:			mips_set_cp0r( mipscpu, 1, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R2:			mips_set_cp0r( mipscpu, 2, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R3:			mips_set_cp0r( mipscpu, 3, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R4:			mips_set_cp0r( mipscpu, 4, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R5:			mips_set_cp0r( mipscpu, 5, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R6:			mips_set_cp0r( mipscpu, 6, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R7:			mips_set_cp0r( mipscpu, 7, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R8:			mips_set_cp0r( mipscpu, 8, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R9:			mips_set_cp0r( mipscpu, 9, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R10:		mips_set_cp0r( mipscpu, 10, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R11:		mips_set_cp0r( mipscpu, 11, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R12:		mips_set_cp0r( mipscpu, 12, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R13:		mips_set_cp0r( mipscpu, 13, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R14:		mips_set_cp0r( mipscpu, 14, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R15:		mips_set_cp0r( mipscpu, 15, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R16:		mips_set_cp0r( mipscpu, 16, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R17:		mips_set_cp0r( mipscpu, 17, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R18:		mips_set_cp0r( mipscpu, 18, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R19:		mips_set_cp0r( mipscpu, 19, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R20:		mips_set_cp0r( mipscpu, 20, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R21:		mips_set_cp0r( mipscpu, 21, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R22:		mips_set_cp0r( mipscpu, 22, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R23:		mips_set_cp0r( mipscpu, 23, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R24:		mips_set_cp0r( mipscpu, 24, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R25:		mips_set_cp0r( mipscpu, 25, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R26:		mips_set_cp0r( mipscpu, 26, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R27:		mips_set_cp0r( mipscpu, 27, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R28:		mips_set_cp0r( mipscpu, 28, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R29:		mips_set_cp0r( mipscpu, 29, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R30:		mips_set_cp0r( mipscpu, 30, info->i );			break;
		case CPUINFO_INT_REGISTER + MIPS_CP0R31:		mips_set_cp0r( mipscpu, 31, info->i );			break;
...
void mips_get_info(MIPS_CPU_CONTEXT *mipscpu, uint32_t state, union cpuinfo *info)
{
	switch (state)
	{
...
uint32_t mips_get_cause(MIPS_CPU_CONTEXT *mipscpu)
{
	return mipscpu->cp0r[ CP0_CAUSE ];
}

uint32_t mips_get_status(MIPS_CPU_CONTEXT *mipscpu)
{
	return mipscpu->cp0r[ CP0_SR ];
}

void mips_set_status(MIPS_CPU_CONTEXT *mipscpu, uint32_t status)
{
	mipscpu->cp0r[ CP0_SR ] = status;
}

uint32_t mips_get_ePC(MIPS_CPU_CONTEXT *mipscpu)
{
...
void psxcpu_get_info(MIPS_CPU_CONTEXT *mipscpu, uint32_t state, union cpuinfo *info)
{
	switch (state)
	{
		/* --- the following bits of info are returned as NULL-terminated strings --- */
